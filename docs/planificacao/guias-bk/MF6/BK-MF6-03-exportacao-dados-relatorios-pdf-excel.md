# BK-MF6-03 - Exportação de dados e relatórios em PDF/Excel
...
Neste BK vamos entregar **exportação de dados e relatórios em pdf/excel** com um resultado final verificável em review e pronto para integração na macro `MF6`.

A execução acontece na Fase 2 e respeita as dependências `BK-MF6-01`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF35, RNF16` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-04` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF6-03` com critérios observáveis.
- Implementar os fluxos principais associados a `RF35, RNF16`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-04` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF35, RNF16`)
- `docs/RNF.md` (secções relacionadas com `RF35, RNF16`)
...
Como fazer: validar `BK-MF6-01` no backlog, confirmar contratos e restrições de `RF35, RNF16`.
...
- Dependências `BK-MF6-01` estão satisfeitas para o contexto atual.
...
- `BK-MF6-04` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
