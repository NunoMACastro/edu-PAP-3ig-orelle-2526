# BK-MF6-02 - Gestão de stock com alertas automáticos
...
Neste BK vamos entregar **gestão de stock com alertas automáticos** com um resultado final verificável em review e pronto para integração na macro `MF6`.
...
- Impacto funcional: habilita diretamente os requisitos `RF32, RNF20` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-03` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF6-02` com critérios observáveis.
- Implementar os fluxos principais associados a `RF32, RNF20`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-03` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF32, RNF20`)
- `docs/RNF.md` (secções relacionadas com `RF32, RNF20`)
...
Como fazer: validar `BK-MF2-01, BK-MF5-02` no backlog, confirmar contratos e restrições de `RF32, RNF20`.
...
- `BK-MF6-03` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
