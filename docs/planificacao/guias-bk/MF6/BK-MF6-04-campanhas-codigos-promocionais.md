# BK-MF6-04 - Campanhas e códigos promocionais
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF2-01, BK-MF5-02`
- `rf_rnf`: `RF42, RF43, RNF01`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **campanhas e códigos promocionais** com um resultado final verificável em review e pronto para integração na macro `MF6`.

A execução acontece na Fase 2 e respeita as dependências `BK-MF2-01, BK-MF5-02`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF42, RF43, RNF01` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-01` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF6-04` com critérios observáveis.
- Implementar os fluxos principais associados a `RF42, RF43, RNF01`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-01` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF42, RF43, RNF01`)
- `docs/RNF.md` (secções relacionadas com `RF42, RF43, RNF01`)
...
Como fazer: validar `BK-MF2-01, BK-MF5-02` no backlog, confirmar contratos e restrições de `RF42, RF43, RNF01`.
...
- Dependências `BK-MF2-01, BK-MF5-02` estão satisfeitas para o contexto atual.
...
- `BK-MF7-01` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
