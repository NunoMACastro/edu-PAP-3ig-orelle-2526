# BK-MF6-01 - Dashboard administrativo com métricas operacionais
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF5-02, BK-MF3-03`
- `rf_rnf`: `RF31, RNF20`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **dashboard administrativo com métricas operacionais** com um resultado final verificável em review e pronto para integração na macro `MF6`.

A execução acontece na Fase 2 e respeita as dependências `BK-MF5-02, BK-MF3-03`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF31, RNF20` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-02` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF6-01` com critérios observáveis.
- Implementar os fluxos principais associados a `RF31, RNF20`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-02` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF31, RNF20`)
- `docs/RNF.md` (secções relacionadas com `RF31, RNF20`)
...
Como fazer: validar `BK-MF5-02, BK-MF3-03` no backlog, confirmar contratos e restrições de `RF31, RNF20`.
...
- Dependências `BK-MF5-02, BK-MF3-03` estão satisfeitas para o contexto atual.
...
- `BK-MF6-02` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
