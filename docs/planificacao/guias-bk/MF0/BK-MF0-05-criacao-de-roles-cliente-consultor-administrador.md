...
## Bloco pedagogico
### Objetivo
Executar `Criação de roles: Cliente, Consultor, Administrador` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RF05` em `docs/RF.md` ou `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF0-05` e do requisito `RF05`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF0-01).
3. Definir contrato de entrada/saida para `Criação de roles: Cliente, Consultor, Administrador`.
...
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF0-05` / `RF05`)**
...
export function executar_bk_mf0_05(input: AnaliseInput) {
