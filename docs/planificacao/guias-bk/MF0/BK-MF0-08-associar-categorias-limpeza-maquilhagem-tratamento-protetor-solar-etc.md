...
## Bloco pedagogico
### Objetivo
Executar `Associar categorias (limpeza, maquilhagem, tratamento, protetor solar, etc.)` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RF08` em `docs/RF.md` ou `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF0-08` e do requisito `RF08`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF0-07).
3. Definir contrato de entrada/saida para `Associar categorias (limpeza, maquilhagem, tratamento, protetor solar, etc.)`.
...
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF0-08` / `RF08`)**
...
export function executar_bk_mf0_08(input: AnaliseInput) {
