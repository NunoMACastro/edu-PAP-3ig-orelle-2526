# BK-MF0-01 - Governança documental e standards de execução
...
- `esforço`: `M`
- `dependências`: `-`
- `rf_rnf`: `RF01-RF44, RNF19, RNF22`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **governança documental e standards de execução** com um resultado final verificável em review e pronto para integração na macro `MF0`.

A execução acontece na Fase 1 e respeita as dependências `-`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF01-RF44, RNF19, RNF22` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-02` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF0-01` com critérios observáveis.
- Implementar os fluxos principais associados a `RF01-RF44, RNF19, RNF22`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-02` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF01-RF44, RNF19, RNF22`)
- `docs/RNF.md` (secções relacionadas com `RF01-RF44, RNF19, RNF22`)
...
Como fazer: validar `-` no backlog, confirmar contratos e restrições de `RF01-RF44, RNF19, RNF22`.
...
- Dependências `-` estão satisfeitas para o contexto atual.
...
- `BK-MF0-02` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
