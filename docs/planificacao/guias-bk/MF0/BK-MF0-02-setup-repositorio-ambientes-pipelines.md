# BK-MF0-02 - Setup de repositório, ambientes e pipelines base
...
Neste BK vamos entregar **setup de repositório, ambientes e pipelines base** com um resultado final verificável em review e pronto para integração na macro `MF0`.

A execução acontece na Fase 1 e respeita as dependências `BK-MF0-01`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RNF19-RNF22` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF0-03` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF0-02` com critérios observáveis.
- Implementar os fluxos principais associados a `RNF19-RNF22`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF0-03` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RNF19-RNF22`)
- `docs/RNF.md` (secções relacionadas com `RNF19-RNF22`)
...
Como fazer: validar `BK-MF0-01` no backlog, confirmar contratos e restrições de `RNF19-RNF22`.
...
- Dependências `BK-MF0-01` estão satisfeitas para o contexto atual.
...
- `BK-MF0-03` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
