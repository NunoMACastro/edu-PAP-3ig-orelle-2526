# BK-MF0-04 - Baseline de segurança técnica (auth, sessão e HTTPS)
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF0-02`
- `rf_rnf`: `RF01, RF02, RNF09, RNF10, RNF14`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **baseline de segurança técnica (auth, sessão e https)** com um resultado final verificável em review e pronto para integração na macro `MF0`.

A execução acontece na Fase 1 e respeita as dependências `BK-MF0-02`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF01, RF02, RNF09, RNF10, RNF14` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF1-01` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF0-04` com critérios observáveis.
- Implementar os fluxos principais associados a `RF01, RF02, RNF09, RNF10, RNF14`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF1-01` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF01, RF02, RNF09, RNF10, RNF14`)
- `docs/RNF.md` (secções relacionadas com `RF01, RF02, RNF09, RNF10, RNF14`)
...
Como fazer: validar `BK-MF0-02` no backlog, confirmar contratos e restrições de `RF01, RF02, RNF09, RNF10, RNF14`.
...
- Dependências `BK-MF0-02` estão satisfeitas para o contexto atual.
...
- `BK-MF1-01` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
