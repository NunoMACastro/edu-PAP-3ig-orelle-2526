# BK-MF7-04 - Privacidade operacional e auditoria biométrica
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF3-01, BK-MF1-04`
- `rf_rnf`: `RF41, RF44, RNF13, RNF25`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **privacidade operacional e auditoria biométrica** com um resultado final verificável em review e pronto para integração na macro `MF7`.

A execução acontece na Fase 3 e respeita as dependências `BK-MF3-01, BK-MF1-04`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF41, RF44, RNF13, RNF25` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-01` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF7-04` com critérios observáveis.
- Implementar os fluxos principais associados a `RF41, RF44, RNF13, RNF25`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-01` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF41, RF44, RNF13, RNF25`)
- `docs/RNF.md` (secções relacionadas com `RF41, RF44, RNF13, RNF25`)
...
Como fazer: validar `BK-MF3-01, BK-MF1-04` no backlog, confirmar contratos e restrições de `RF41, RF44, RNF13, RNF25`.
...
- Dependências `BK-MF3-01, BK-MF1-04` estão satisfeitas para o contexto atual.
...
- `BK-MF8-01` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
