# BK-MF7-02 - Chat interno cliente-consultor
...
Neste BK vamos entregar **chat interno cliente-consultor** com um resultado final verificável em review e pronto para integração na macro `MF7`.

A execução acontece na Fase 3 e respeita as dependências `BK-MF1-01`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF38, RNF06` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-03` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF7-02` com critérios observáveis.
- Implementar os fluxos principais associados a `RF38, RNF06`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-03` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF38, RNF06`)
- `docs/RNF.md` (secções relacionadas com `RF38, RNF06`)
...
Como fazer: validar `BK-MF1-01` no backlog, confirmar contratos e restrições de `RF38, RNF06`.
...
- `BK-MF7-03` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
