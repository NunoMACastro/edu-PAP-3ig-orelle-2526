# BK-MF7-03 - Simulação virtual antes/depois
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF3-01, BK-MF2-02`
- `rf_rnf`: `RF23, RF24, RNF05`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **simulação virtual antes/depois** com um resultado final verificável em review e pronto para integração na macro `MF7`.

A execução acontece na Fase 3 e respeita as dependências `BK-MF3-01, BK-MF2-02`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF23, RF24, RNF05` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-04` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF7-03` com critérios observáveis.
- Implementar os fluxos principais associados a `RF23, RF24, RNF05`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-04` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF23, RF24, RNF05`)
- `docs/RNF.md` (secções relacionadas com `RF23, RF24, RNF05`)
...
Como fazer: validar `BK-MF3-01, BK-MF2-02` no backlog, confirmar contratos e restrições de `RF23, RF24, RNF05`.
...
- Dependências `BK-MF3-01, BK-MF2-02` estão satisfeitas para o contexto atual.
...
- `BK-MF7-04` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
