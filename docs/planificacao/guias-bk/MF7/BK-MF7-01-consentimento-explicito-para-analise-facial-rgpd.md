...
## Contexto do BK
- Entrega alvo: implementar `Consentimento explícito para análise facial (RGPD)` com rastreabilidade direta ao requisito `RNF12`.
- Foco tecnico da macro: `Privacidade, seguranca e controlo`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `Consentimento explícito para análise facial (RGPD)` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RNF12` em `docs/RF.md` ou `docs/RNF.md`.
...
3. Definir contrato de entrada/saida para `Consentimento explícito para análise facial (RGPD)`.
...
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF7-01` / `RNF12`)**
...
export function executar_bk_mf7_01(input: AnaliseInput) {
