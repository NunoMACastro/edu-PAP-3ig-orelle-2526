# BK-MF7-01 - Notificações transacionais e preferências de comunicação
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF5-02, BK-MF1-02`
- `rf_rnf`: `RF36, RF39, RNF03`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **notificações transacionais e preferências de comunicação** com um resultado final verificável em review e pronto para integração na macro `MF7`.

A execução acontece na Fase 3 e respeita as dependências `BK-MF5-02, BK-MF1-02`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF36, RF39, RNF03` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF7-02` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF7-01` com critérios observáveis.
- Implementar os fluxos principais associados a `RF36, RF39, RNF03`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF7-02` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF36, RF39, RNF03`)
- `docs/RNF.md` (secções relacionadas com `RF36, RF39, RNF03`)
...
Como fazer: validar `BK-MF5-02, BK-MF1-02` no backlog, confirmar contratos e restrições de `RF36, RF39, RNF03`.
...
- Dependências `BK-MF5-02, BK-MF1-02` estão satisfeitas para o contexto atual.
...
- `BK-MF7-02` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
