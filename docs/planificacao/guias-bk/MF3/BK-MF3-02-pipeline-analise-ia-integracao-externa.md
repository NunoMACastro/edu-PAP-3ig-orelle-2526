# BK-MF3-02 - Pipeline de análise IA e integração com serviço externo
...
- `estado`: `TODO`
- `esforço`: `L`
- `dependências`: `BK-MF3-01, BK-MF0-02`
- `rf_rnf`: `RF14, RNF05, RNF18`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **pipeline de análise ia e integração com serviço externo** com um resultado final verificável em review e pronto para integração na macro `MF3`.

A execução acontece na Fase 1 e respeita as dependências `BK-MF3-01, BK-MF0-02`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF14, RNF05, RNF18` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-03` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF3-02` com critérios observáveis.
- Implementar os fluxos principais associados a `RF14, RNF05, RNF18`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-03` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF14, RNF05, RNF18`)
- `docs/RNF.md` (secções relacionadas com `RF14, RNF05, RNF18`)
...
Como fazer: validar `BK-MF3-01, BK-MF0-02` no backlog, confirmar contratos e restrições de `RF14, RNF05, RNF18`.
...
- Dependências `BK-MF3-01, BK-MF0-02` estão satisfeitas para o contexto atual.
...
- `BK-MF3-03` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
