# BK-MF3-01 - Upload de fotografia com consentimento e armazenamento seguro
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF1-02`
- `rf_rnf`: `RF13, RNF11, RNF12`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **upload de fotografia com consentimento e armazenamento seguro** com um resultado final verificável em review e pronto para integração na macro `MF3`.

A execução acontece na Fase 1 e respeita as dependências `BK-MF1-02`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF13, RNF11, RNF12` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-02` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF3-01` com critérios observáveis.
- Implementar os fluxos principais associados a `RF13, RNF11, RNF12`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-02` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF13, RNF11, RNF12`)
- `docs/RNF.md` (secções relacionadas com `RF13, RNF11, RNF12`)
...
Como fazer: validar `BK-MF1-02` no backlog, confirmar contratos e restrições de `RF13, RNF11, RNF12`.
...
- Dependências `BK-MF1-02` estão satisfeitas para o contexto atual.
...
- `BK-MF3-02` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
