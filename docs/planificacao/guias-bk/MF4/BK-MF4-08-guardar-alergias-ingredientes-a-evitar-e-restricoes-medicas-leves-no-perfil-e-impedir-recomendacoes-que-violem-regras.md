...
## Contexto do BK
- Entrega alvo: implementar `Guardar alergias, ingredientes a evitar e restrições médicas leves no perfil e impedir recomendações que violem regras` com rastreabilidade direta ao requisito `RF40`.
- Foco tecnico da macro: `Capacidades de produto II`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `Guardar alergias, ingredientes a evitar e restrições médicas leves no perfil e impedir recomendações que violem regras` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RF40` em `docs/RF.md` ou `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF4-08` e do requisito `RF40`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF0-03).
3. Definir contrato de entrada/saida para `Guardar alergias, ingredientes a evitar e restrições médicas leves no perfil e impedir recomendações que violem regras`.
...
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF4-08` / `RF40`)**

```ts
const BK_ID = 'BK-MF4-08';
const REQ_ID = 'RF40';

type AnaliseInput = { userId: string; imagemId?: string; perfilId?: string };

export function executar_bk_mf4_08(input: AnaliseInput) {
