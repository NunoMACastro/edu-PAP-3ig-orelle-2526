# BK-MF4-04 - Revisão manual das recomendações por consultor
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF4-01, BK-MF1-01`
- `rf_rnf`: `RF22, RNF23`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **revisão manual das recomendações por consultor** com um resultado final verificável em review e pronto para integração na macro `MF4`.

A execução acontece na Fase 2 e respeita as dependências `BK-MF4-01, BK-MF1-01`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF22, RNF23` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF5-01` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF4-04` com critérios observáveis.
- Implementar os fluxos principais associados a `RF22, RNF23`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF5-01` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF22, RNF23`)
- `docs/RNF.md` (secções relacionadas com `RF22, RNF23`)
...
Como fazer: validar `BK-MF4-01, BK-MF1-01` no backlog, confirmar contratos e restrições de `RF22, RNF23`.
...
- Dependências `BK-MF4-01, BK-MF1-01` estão satisfeitas para o contexto atual.
...
- `BK-MF5-01` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
