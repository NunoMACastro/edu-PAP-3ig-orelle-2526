# BK-MF4-03 - Rotinas diárias e alertas personalizados
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF4-01`
- `rf_rnf`: `RF21, RF37, RNF03`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **rotinas diárias e alertas personalizados** com um resultado final verificável em review e pronto para integração na macro `MF4`.

A execução acontece na Fase 2 e respeita as dependências `BK-MF4-01`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF21, RF37, RNF03` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF4-04` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF4-03` com critérios observáveis.
- Implementar os fluxos principais associados a `RF21, RF37, RNF03`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF4-04` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF21, RF37, RNF03`)
- `docs/RNF.md` (secções relacionadas com `RF21, RF37, RNF03`)
...
Como fazer: validar `BK-MF4-01` no backlog, confirmar contratos e restrições de `RF21, RF37, RNF03`.
...
- Dependências `BK-MF4-01` estão satisfeitas para o contexto atual.
...
- `BK-MF4-04` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
