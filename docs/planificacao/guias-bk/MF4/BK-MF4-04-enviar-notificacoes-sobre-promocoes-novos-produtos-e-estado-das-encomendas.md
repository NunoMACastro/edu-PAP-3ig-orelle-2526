...
## Contexto do BK
- Entrega alvo: implementar `Enviar notificações sobre promoções, novos produtos e estado das encomendas` com rastreabilidade direta ao requisito `RF36`.
- Foco tecnico da macro: `Capacidades de produto II`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `Enviar notificações sobre promoções, novos produtos e estado das encomendas` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RF36` em `docs/RF.md` ou `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF4-04` e do requisito `RF36`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF3-03).
3. Definir contrato de entrada/saida para `Enviar notificações sobre promoções, novos produtos e estado das encomendas`.
...
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF4-04` / `RF36`)**

```ts
const BK_ID = 'BK-MF4-04';
const REQ_ID = 'RF36';

type AnaliseInput = { userId: string; imagemId?: string; perfilId?: string };

export function executar_bk_mf4_04(input: AnaliseInput) {
