# BK-MF4-01 - Motor de recomendação personalizada com motivo de sugestão
...
- `estado`: `TODO`
- `esforço`: `L`
- `dependências`: `BK-MF3-03, BK-MF2-02`
- `rf_rnf`: `RF18, RF19, RNF24`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **motor de recomendação personalizada com motivo de sugestão** com um resultado final verificável em review e pronto para integração na macro `MF4`.

A execução acontece na Fase 2 e respeita as dependências `BK-MF3-03, BK-MF2-02`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF18, RF19, RNF24` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF4-02` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF4-01` com critérios observáveis.
- Implementar os fluxos principais associados a `RF18, RF19, RNF24`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF4-02` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF18, RF19, RNF24`)
- `docs/RNF.md` (secções relacionadas com `RF18, RF19, RNF24`)
...
Como fazer: validar `BK-MF3-03, BK-MF2-02` no backlog, confirmar contratos e restrições de `RF18, RF19, RNF24`.
...
- Dependências `BK-MF3-03, BK-MF2-02` estão satisfeitas para o contexto atual.
...
- `BK-MF4-02` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
