# BK-MF8-04 - Hardening final, testes negativos e preparação da defesa
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF8-01, BK-MF8-02, BK-MF8-03, BK-MF7-04`
- `rf_rnf`: `RF01-RF44, RNF09-RNF25`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **hardening final, testes negativos e preparação da defesa** com um resultado final verificável em review e pronto para integração na macro `MF8`.

A execução acontece na Fase 3 e respeita as dependências `BK-MF8-01, BK-MF8-02, BK-MF8-03, BK-MF7-04`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF01-RF44, RNF09-RNF25` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `-` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF8-04` com critérios observáveis.
- Implementar os fluxos principais associados a `RF01-RF44, RNF09-RNF25`.
...
- Trabalho de BKs seguintes, incluindo `-` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF01-RF44, RNF09-RNF25`)
- `docs/RNF.md` (secções relacionadas com `RF01-RF44, RNF09-RNF25`)
...
Como fazer: validar `BK-MF8-01, BK-MF8-02, BK-MF8-03, BK-MF7-04` no backlog, confirmar contratos e restrições de `RF01-RF44, RNF09-RNF25`.
...
- Dependências `BK-MF8-01, BK-MF8-02, BK-MF8-03, BK-MF7-04` estão satisfeitas para o contexto atual.
...
- `-` - Último BK da cadeia; próximo passo é consolidação final e defesa.
