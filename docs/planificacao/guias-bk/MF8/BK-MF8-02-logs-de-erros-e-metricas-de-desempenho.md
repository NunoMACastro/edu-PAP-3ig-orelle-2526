...
## Contexto do BK
- Entrega alvo: implementar `Logs de erros e métricas de desempenho` com rastreabilidade direta ao requisito `RNF20`.
- Foco tecnico da macro: `Integracoes, compatibilidade e fecho`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `Logs de erros e métricas de desempenho` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RNF20` em `docs/RF.md` ou `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF8-02` e do requisito `RNF20`.
2. Validar pre-condicoes e dependencias declaradas (-).
3. Definir contrato de entrada/saida para `Logs de erros e métricas de desempenho`.
...
**Snippet tecnico orientado ao dominio hibrido (consultoria + monetizacao) (`BK-MF8-02` / `RNF20`)**

```ts
const BK_ID = 'BK-MF8-02';
const REQ_ID = 'RNF20';

type EventoHibrido = { userId: string; recomendacaoId?: string; acao?: 'VIEW' | 'CART' | 'BUY' };

export function executar_bk_mf8_02(evento: EventoHibrido) {
...
## Criterios de aceite
- Entrega funcional especifica de `Logs de erros e métricas de desempenho` validada contra `RNF20`.
