...
## Bloco pedagogico
### Objetivo
O aluno deve garantir que testes, dados de demonstração e variáveis de configuração não se misturam com produção.

### Pre-requisitos
- Rever `RNF22` em `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF8-03` e do requisito `RNF22`.
...
**Snippet tecnico orientado a ambientes/testes (`BK-MF8-03` / `RNF22`)**
...
export function validarFechoMf8(evidence) {
  ...
  return {
    ...
    requisito: REQ_ID,
    estado: 'validado',
    dominio: 'ambientes e configuracao',
  };
}
```

## Checklist tecnico especifico
- ficheiros/env de teste e produção estão separados
- os testes usam base de dados/serviços próprios ou controlados
- nenhuma credencial real é necessária para executar validação local
