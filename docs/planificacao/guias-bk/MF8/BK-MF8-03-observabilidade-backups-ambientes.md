# BK-MF8-03 - Observabilidade, backups e separação de ambientes
...
- `rf_rnf`: `RNF20, RNF21, RNF22`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **observabilidade, backups e separação de ambientes** com um resultado final verificável em review e pronto para integração na macro `MF8`.

A execução acontece na Fase 3 e respeita as dependências `BK-MF0-02`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RNF20, RNF21, RNF22` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-04` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF8-03` com critérios observáveis.
- Implementar os fluxos principais associados a `RNF20, RNF21, RNF22`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-04` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RNF20, RNF21, RNF22`)
- `docs/RNF.md` (secções relacionadas com `RNF20, RNF21, RNF22`)
...
Como fazer: validar `BK-MF0-02` no backlog, confirmar contratos e restrições de `RNF20, RNF21, RNF22`.
...
- `BK-MF8-04` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
