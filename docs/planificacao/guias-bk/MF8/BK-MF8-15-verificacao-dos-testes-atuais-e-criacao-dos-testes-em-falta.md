...
## Contexto do BK
- Entrega alvo: implementar `Verificação dos testes atuais e criação dos testes em falta.` com rastreabilidade direta ao requisito `RNF27`.
- Foco tecnico da macro: `Fecho visual, qualidade, testes finais e estabilizacao`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
O aluno deve inventariar os testes existentes, identificar lacunas nos fluxos críticos e criar os testes mínimos em falta antes da bateria final.

### Pre-requisitos
- Rever `RNF27` em `docs/RNF.md`.
- Validar linha do BK no `BACKLOG-MVP.md` e na `MATRIZ-CANONICA-BK.md`.
- Confirmar dependencias declaradas: `BK-MF8-03, BK-MF8-14`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF8-15` e do requisito `RNF27`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF8-03, BK-MF8-14).
3. Definir contrato de entrada/saida para `Verificação dos testes atuais e criação dos testes em falta.`.
...
**Snippet tecnico orientado a qa/cobertura (`BK-MF8-15` / `RNF27`)**
...
export function validarFechoMf8(evidence) {
  ...
  return {
    ...
    requisito: REQ_ID,
    estado: 'validado',
    dominio: 'cobertura de testes',
  };
}
```

## Checklist tecnico especifico
- há uma lista de testes existentes por camada
- lacunas críticas ficam associadas a testes novos ou justificação
- testes novos cobrem fluxos positivos e negativos relevantes

## Criterios de aceite
- Entrega funcional especifica de `Verificação dos testes atuais e criação dos testes em falta.` validada contra `RNF27`.
...
## Changelog
- `2026-06-30`: guia renumerado para `BK-MF8-15` para manter os testes como fecho final da MF8.
