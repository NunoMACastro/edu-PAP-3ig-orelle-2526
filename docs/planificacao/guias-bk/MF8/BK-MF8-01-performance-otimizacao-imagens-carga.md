# BK-MF8-01 - Performance, otimização de imagens e testes de carga
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF3-02, BK-MF5-02`
- `rf_rnf`: `RNF05, RNF06, RNF07, RNF08`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **performance, otimização de imagens e testes de carga** com um resultado final verificável em review e pronto para integração na macro `MF8`.

A execução acontece na Fase 3 e respeita as dependências `BK-MF3-02, BK-MF5-02`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RNF05, RNF06, RNF07, RNF08` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF8-02` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF8-01` com critérios observáveis.
- Implementar os fluxos principais associados a `RNF05, RNF06, RNF07, RNF08`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF8-02` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RNF05, RNF06, RNF07, RNF08`)
- `docs/RNF.md` (secções relacionadas com `RNF05, RNF06, RNF07, RNF08`)
...
Como fazer: validar `BK-MF3-02, BK-MF5-02` no backlog, confirmar contratos e restrições de `RNF05, RNF06, RNF07, RNF08`.
...
- Dependências `BK-MF3-02, BK-MF5-02` estão satisfeitas para o contexto atual.
...
- `BK-MF8-02` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
