# BK-MF5-01 - Carrinho de compras e gestão de itens
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF2-02, BK-MF1-01`
- `rf_rnf`: `RF26, RNF06`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **carrinho de compras e gestão de itens** com um resultado final verificável em review e pronto para integração na macro `MF5`.

A execução acontece na Fase 2 e respeita as dependências `BK-MF2-02, BK-MF1-01`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF26, RNF06` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF5-02` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF5-01` com critérios observáveis.
- Implementar os fluxos principais associados a `RF26, RNF06`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF5-02` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF26, RNF06`)
- `docs/RNF.md` (secções relacionadas com `RF26, RNF06`)
...
Como fazer: validar `BK-MF2-02, BK-MF1-01` no backlog, confirmar contratos e restrições de `RF26, RNF06`.
...
- Dependências `BK-MF2-02, BK-MF1-01` estão satisfeitas para o contexto atual.
...
- `BK-MF5-02` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
