# BK-MF5-04 - Devoluções e trocas com rastreio do motivo
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF5-03`
- `rf_rnf`: `RF29, RNF03`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **devoluções e trocas com rastreio do motivo** com um resultado final verificável em review e pronto para integração na macro `MF5`.

A execução acontece na Fase 2 e respeita as dependências `BK-MF5-03`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF29, RNF03` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF6-01` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF5-04` com critérios observáveis.
- Implementar os fluxos principais associados a `RF29, RNF03`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF6-01` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF29, RNF03`)
- `docs/RNF.md` (secções relacionadas com `RF29, RNF03`)
...
Como fazer: validar `BK-MF5-03` no backlog, confirmar contratos e restrições de `RF29, RNF03`.
...
- Dependências `BK-MF5-03` estão satisfeitas para o contexto atual.
...
- `BK-MF6-01` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
