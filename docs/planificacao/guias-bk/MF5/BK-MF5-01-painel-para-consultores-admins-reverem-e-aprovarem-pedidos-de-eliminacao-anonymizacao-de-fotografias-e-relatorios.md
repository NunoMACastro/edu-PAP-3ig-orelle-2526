...
## Contexto do BK
- Entrega alvo: implementar `Painel para consultores/admins reverem e aprovarem pedidos de eliminação/anonymização de fotografias e relatórios` com rastreabilidade direta ao requisito `RF41`.
- Foco tecnico da macro: `Operacao e fluxos transversais`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `Painel para consultores/admins reverem e aprovarem pedidos de eliminação/anonymização de fotografias e relatórios` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RF41` em `docs/RF.md` ou `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF5-01` e do requisito `RF41`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF1-05).
3. Definir contrato de entrada/saida para `Painel para consultores/admins reverem e aprovarem pedidos de eliminação/anonymização de fotografias e relatórios`.
...
### Handoff
- Proximo BK recomendado: `BK-MF5-04`
- Registar no handoff estado de dependencias, riscos e decisao tecnica tomada.
- Se houver bloqueio >48h, escalar no scorecard da sprint.

## Snippet tecnico aplicavel
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF5-01` / `RF41`)**

```ts
const BK_ID = 'BK-MF5-01';
const REQ_ID = 'RF41';

type AnaliseInput = { userId: string; imagemId?: string; perfilId?: string };

export function executar_bk_mf5_01(input: AnaliseInput) {
...
## Criterios de aceite
- Entrega funcional especifica de `Painel para consultores/admins reverem e aprovarem pedidos de eliminação/anonymização de fotografias e relatórios` validada contra `RF41`.
