...
## Contexto do BK
- Entrega alvo: implementar `Interface moderna, intuitiva e _responsive_ (desktop e mobile)` com rastreabilidade direta ao requisito `RNF01`.
- Foco tecnico da macro: `Operacao e fluxos transversais`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `Interface moderna, intuitiva e _responsive_ (desktop e mobile)` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RNF01` em `docs/RF.md` ou `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF5-05` e do requisito `RNF01`.
2. Validar pre-condicoes e dependencias declaradas (-).
3. Definir contrato de entrada/saida para `Interface moderna, intuitiva e _responsive_ (desktop e mobile)`.
...
**Snippet tecnico orientado ao dominio hibrido (consultoria + monetizacao) (`BK-MF5-05` / `RNF01`)**

```ts
const BK_ID = 'BK-MF5-05';
const REQ_ID = 'RNF01';

type EventoHibrido = { userId: string; recomendacaoId?: string; acao?: 'VIEW' | 'CART' | 'BUY' };

export function executar_bk_mf5_05(evento: EventoHibrido) {
...
## Criterios de aceite
- Entrega funcional especifica de `Interface moderna, intuitiva e _responsive_ (desktop e mobile)` validada contra `RNF01`.
