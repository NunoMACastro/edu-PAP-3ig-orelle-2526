# BK-MF5-02 - Checkout, encomendas e integração de pagamentos
...
- `rf_rnf`: `RF27, RNF09, RNF17`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **checkout, encomendas e integração de pagamentos** com um resultado final verificável em review e pronto para integração na macro `MF5`.

A execução acontece na Fase 2 e respeita as dependências `BK-MF5-01`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF27, RNF09, RNF17` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF5-03` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF5-02` com critérios observáveis.
- Implementar os fluxos principais associados a `RF27, RNF09, RNF17`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF5-03` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF27, RNF09, RNF17`)
- `docs/RNF.md` (secções relacionadas com `RF27, RNF09, RNF17`)
...
Como fazer: validar `BK-MF5-01` no backlog, confirmar contratos e restrições de `RF27, RNF09, RNF17`.
...
- Dependências `BK-MF5-01` estão satisfeitas para o contexto atual.
...
- `BK-MF5-03` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
