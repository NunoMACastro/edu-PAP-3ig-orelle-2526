...
## Contexto do BK
- Entrega alvo: implementar `Registo/auditoria de acessos a dados biométricos, com alertas para usos indevidos` com rastreabilidade direta ao requisito `RF44`.
- Foco tecnico da macro: `Operacao e fluxos transversais`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `Registo/auditoria de acessos a dados biométricos, com alertas para usos indevidos` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RF44` em `docs/RF.md` ou `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF5-04` e do requisito `RF44`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF1-05).
3. Definir contrato de entrada/saida para `Registo/auditoria de acessos a dados biométricos, com alertas para usos indevidos`.
...
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF5-04` / `RF44`)**

```ts
const BK_ID = 'BK-MF5-04';
const REQ_ID = 'RF44';

type AnaliseInput = { userId: string; imagemId?: string; perfilId?: string };

export function executar_bk_mf5_04(input: AnaliseInput) {
...
## Criterios de aceite
- Entrega funcional especifica de `Registo/auditoria de acessos a dados biométricos, com alertas para usos indevidos` validada contra `RF44`.
