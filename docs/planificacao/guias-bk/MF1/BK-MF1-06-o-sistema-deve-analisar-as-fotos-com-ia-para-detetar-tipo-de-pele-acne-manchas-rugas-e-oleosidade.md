...
## Contexto do BK
- Entrega alvo: implementar `O sistema deve analisar as fotos com IA para detetar tipo de pele, acne, manchas, rugas e oleosidade` com rastreabilidade direta ao requisito `RF14`.
- Foco tecnico da macro: `Nucleo funcional I`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `O sistema deve analisar as fotos com IA para detetar tipo de pele, acne, manchas, rugas e oleosidade` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RF14` em `docs/RF.md` ou `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF1-06` e do requisito `RF14`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF1-05).
3. Definir contrato de entrada/saida para `O sistema deve analisar as fotos com IA para detetar tipo de pele, acne, manchas, rugas e oleosidade`.
...
### Handoff
- Proximo BK recomendado: `BK-MF1-07`
- Registar no handoff estado de dependencias, riscos e decisao tecnica tomada.
- Se houver bloqueio >48h, escalar no scorecard da sprint.

## Snippet tecnico aplicavel
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF1-06` / `RF14`)**

```ts
const BK_ID = 'BK-MF1-06';
const REQ_ID = 'RF14';

type AnaliseInput = { userId: string; imagemId?: string; perfilId?: string };

export function executar_bk_mf1_06(input: AnaliseInput) {
...
## Criterios de aceite
- Entrega funcional especifica de `O sistema deve analisar as fotos com IA para detetar tipo de pele, acne, manchas, rugas e oleosidade` validada contra `RF14`.
