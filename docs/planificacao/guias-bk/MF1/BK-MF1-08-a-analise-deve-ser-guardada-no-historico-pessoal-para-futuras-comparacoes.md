...
## Contexto do BK
- Entrega alvo: implementar `A análise deve ser guardada no histórico pessoal para futuras comparações` com rastreabilidade direta ao requisito `RF16`.
- Foco tecnico da macro: `Nucleo funcional I`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `A análise deve ser guardada no histórico pessoal para futuras comparações` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RF16` em `docs/RF.md` ou `docs/RNF.md`.
- Validar linha do BK no `BACKLOG-MVP.md` e na `MATRIZ-CANONICA-BK.md`.
- Confirmar dependencias declaradas: `BK-MF1-07`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF1-08` e do requisito `RF16`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF1-07).
3. Definir contrato de entrada/saida para `A análise deve ser guardada no histórico pessoal para futuras comparações`.
...
### Handoff
- Proximo BK recomendado: `BK-MF2-01`
- Registar no handoff estado de dependencias, riscos e decisao tecnica tomada.
- Se houver bloqueio >48h, escalar no scorecard da sprint.

## Snippet tecnico aplicavel
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF1-08` / `RF16`)**

```ts
const BK_ID = 'BK-MF1-08';
const REQ_ID = 'RF16';

type AnaliseInput = { userId: string; imagemId?: string; perfilId?: string };

export function executar_bk_mf1_08(input: AnaliseInput) {
...
## Criterios de aceite
- Entrega funcional especifica de `A análise deve ser guardada no histórico pessoal para futuras comparações` validada contra `RF16`.
