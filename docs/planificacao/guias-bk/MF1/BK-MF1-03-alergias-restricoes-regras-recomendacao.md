# BK-MF1-03 - Alergias, restrições e regras de segurança de recomendação
...
Neste BK vamos entregar **alergias, restrições e regras de segurança de recomendação** com um resultado final verificável em review e pronto para integração na macro `MF1`.
...
- Impacto funcional: habilita diretamente os requisitos `RF40, RNF24` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF1-04` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF1-03` com critérios observáveis.
...
- Trabalho de BKs seguintes, incluindo `BK-MF1-04` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF40, RNF24`)
- `docs/RNF.md` (secções relacionadas com `RF40, RNF24`)
...
Como fazer: validar `BK-MF1-02` no backlog, confirmar contratos e restrições de `RF40, RNF24`.
...
- `BK-MF1-04` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
