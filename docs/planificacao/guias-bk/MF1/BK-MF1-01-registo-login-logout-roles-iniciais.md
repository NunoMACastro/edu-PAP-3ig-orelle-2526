# BK-MF1-01 - Registo, login, logout e roles iniciais
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF0-04`
- `rf_rnf`: `RF01, RF02, RF05, RNF10, RNF14`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **registo, login, logout e roles iniciais** com um resultado final verificável em review e pronto para integração na macro `MF1`.

A execução acontece na Fase 1 e respeita as dependências `BK-MF0-04`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF01, RF02, RF05, RNF10, RNF14` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF1-02` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF1-01` com critérios observáveis.
- Implementar os fluxos principais associados a `RF01, RF02, RF05, RNF10, RNF14`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF1-02` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF01, RF02, RF05, RNF10, RNF14`)
- `docs/RNF.md` (secções relacionadas com `RF01, RF02, RF05, RNF10, RNF14`)
...
Como fazer: validar `BK-MF0-04` no backlog, confirmar contratos e restrições de `RF01, RF02, RF05, RNF10, RNF14`.
...
- Dependências `BK-MF0-04` estão satisfeitas para o contexto atual.
...
- `BK-MF1-02` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
