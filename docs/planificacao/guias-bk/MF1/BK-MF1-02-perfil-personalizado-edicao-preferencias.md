# BK-MF1-02 - Perfil personalizado com edição e preferências base
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF1-01`
- `rf_rnf`: `RF03, RF04, RF06, RNF01, RNF03`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **perfil personalizado com edição e preferências base** com um resultado final verificável em review e pronto para integração na macro `MF1`.

A execução acontece na Fase 1 e respeita as dependências `BK-MF1-01`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF03, RF04, RF06, RNF01, RNF03` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF1-03` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF1-02` com critérios observáveis.
- Implementar os fluxos principais associados a `RF03, RF04, RF06, RNF01, RNF03`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF1-03` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF03, RF04, RF06, RNF01, RNF03`)
- `docs/RNF.md` (secções relacionadas com `RF03, RF04, RF06, RNF01, RNF03`)
...
Como fazer: validar `BK-MF1-01` no backlog, confirmar contratos e restrições de `RF03, RF04, RF06, RNF01, RNF03`.
...
- Dependências `BK-MF1-01` estão satisfeitas para o contexto atual.
...
- `BK-MF1-03` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
