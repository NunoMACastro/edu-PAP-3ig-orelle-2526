# BK-MF1-04 - Gestão administrativa de contas (ativar, suspender, eliminar)
...
Neste BK vamos entregar **gestão administrativa de contas (ativar, suspender, eliminar)** com um resultado final verificável em review e pronto para integração na macro `MF1`.
...
- Impacto funcional: habilita diretamente os requisitos `RF33, RNF13` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-01` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF1-04` com critérios observáveis.
- Implementar os fluxos principais associados a `RF33, RNF13`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-01` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF33, RNF13`)
- `docs/RNF.md` (secções relacionadas com `RF33, RNF13`)
...
Como fazer: validar `BK-MF1-01` no backlog, confirmar contratos e restrições de `RF33, RNF13`.
...
- `BK-MF2-01` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
