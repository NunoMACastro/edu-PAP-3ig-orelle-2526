...
## Contexto do BK
- Entrega alvo: implementar `A IA deve gerar uma visualização antes/depois com os produtos recomendados` com rastreabilidade direta ao requisito `RF24`.
- Foco tecnico da macro: `Nucleo funcional II`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `A IA deve gerar uma visualização antes/depois com os produtos recomendados` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RF24` em `docs/RF.md` ou `docs/RNF.md`.
- Validar linha do BK no `BACKLOG-MVP.md` e na `MATRIZ-CANONICA-BK.md`.
- Confirmar dependencias declaradas: `BK-MF2-07`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF2-08` e do requisito `RF24`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF2-07).
3. Definir contrato de entrada/saida para `A IA deve gerar uma visualização antes/depois com os produtos recomendados`.
...
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF2-08` / `RF24`)**

```ts
const BK_ID = 'BK-MF2-08';
const REQ_ID = 'RF24';

type AnaliseInput = { userId: string; imagemId?: string; perfilId?: string };

export function executar_bk_mf2_08(input: AnaliseInput) {
...
## Criterios de aceite
- Entrega funcional especifica de `A IA deve gerar uma visualização antes/depois com os produtos recomendados` validada contra `RF24`.
