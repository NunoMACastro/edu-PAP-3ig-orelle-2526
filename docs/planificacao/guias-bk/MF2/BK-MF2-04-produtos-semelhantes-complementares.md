# BK-MF2-04 - Produtos semelhantes e complementares no catálogo
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF2-02`
- `rf_rnf`: `RF12, RNF23`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **produtos semelhantes e complementares no catálogo** com um resultado final verificável em review e pronto para integração na macro `MF2`.

A execução acontece na Fase 1 e respeita as dependências `BK-MF2-02`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF12, RNF23` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF3-01` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF2-04` com critérios observáveis.
- Implementar os fluxos principais associados a `RF12, RNF23`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF3-01` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF12, RNF23`)
- `docs/RNF.md` (secções relacionadas com `RF12, RNF23`)
...
Como fazer: validar `BK-MF2-02` no backlog, confirmar contratos e restrições de `RF12, RNF23`.
...
- Dependências `BK-MF2-02` estão satisfeitas para o contexto atual.
...
- `BK-MF3-01` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
