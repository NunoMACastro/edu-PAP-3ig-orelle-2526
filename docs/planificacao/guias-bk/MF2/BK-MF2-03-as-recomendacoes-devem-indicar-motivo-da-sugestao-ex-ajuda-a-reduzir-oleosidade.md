# BK-MF2-03 - As recomendações devem indicar motivo da sugestão (ex: “ajuda a reduzir oleosidade”)
...
## Contexto do BK
- Entrega alvo: implementar `As recomendações devem indicar motivo da sugestão (ex: “ajuda a reduzir oleosidade”)` com rastreabilidade direta ao requisito `RF19`.
- Foco tecnico da macro: `Nucleo funcional II`.
- Regra de governanca: preservar IDs BK, contrato de campos e consistencia entre backlog, matriz, sprints e guias.

## Bloco pedagogico
### Objetivo
Executar `As recomendações devem indicar motivo da sugestão (ex: “ajuda a reduzir oleosidade”)` com evidência tecnica objetiva e fecho documental alinhado ao contrato canónico.

### Pre-requisitos
- Rever `RF19` em `docs/RF.md` ou `docs/RNF.md`.
...
### Passos
1. Confirmar no backlog e na matriz o contexto do `BK-MF2-03` e do requisito `RF19`.
2. Validar pre-condicoes e dependencias declaradas (BK-MF2-02).
3. Definir contrato de entrada/saida para `As recomendações devem indicar motivo da sugestão (ex: “ajuda a reduzir oleosidade”)`.
...
**Snippet tecnico orientado ao dominio de consultoria inteligente (`BK-MF2-03` / `RF19`)**

```ts
const BK_ID = 'BK-MF2-03';
const REQ_ID = 'RF19';

type AnaliseInput = { userId: string; imagemId?: string; perfilId?: string };

export function executar_bk_mf2_03(input: AnaliseInput) {
...
## Criterios de aceite
- Entrega funcional especifica de `As recomendações devem indicar motivo da sugestão (ex: “ajuda a reduzir oleosidade”)` validada contra `RF19`.
