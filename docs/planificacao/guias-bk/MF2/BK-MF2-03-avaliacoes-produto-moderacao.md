# BK-MF2-03 - Avaliações de produto e moderação de conteúdo
...
- `estado`: `TODO`
- `esforço`: `S`
- `dependências`: `BK-MF2-02, BK-MF1-04`
- `rf_rnf`: `RF11, RF34, RNF03`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar **avaliações de produto e moderação de conteúdo** com um resultado final verificável em review e pronto para integração na macro `MF2`.

A execução acontece na Fase 1 e respeita as dependências `BK-MF2-02, BK-MF1-04`. Qualquer detalhe técnico não fechado fica marcado como **a definir no BK dependente**, sem expandir scope fora deste BK.

## Porque isto é importante
- Impacto funcional: habilita diretamente os requisitos `RF11, RF34, RNF03` com entrega concreta visível no produto.
...
- Impacto na sequência: desbloqueia a progressão para `BK-MF2-04` sem ambiguidade documental.
- Risco se mal executado: cria falhas de integração, regressões de qualidade ou atraso em cadeia na macro seguinte.

## O que entra (scope)
- Definir o entregável técnico e funcional de `BK-MF2-03` com critérios observáveis.
- Implementar os fluxos principais associados a `RF11, RF34, RNF03`.
...
- Trabalho de BKs seguintes, incluindo `BK-MF2-04` quando aplicável.
...
- `docs/RF.md` (secções relacionadas com `RF11, RF34, RNF03`)
- `docs/RNF.md` (secções relacionadas com `RF11, RF34, RNF03`)
...
Como fazer: validar `BK-MF2-02, BK-MF1-04` no backlog, confirmar contratos e restrições de `RF11, RF34, RNF03`.
...
- Dependências `BK-MF2-02, BK-MF1-04` estão satisfeitas para o contexto atual.
...
- `BK-MF2-04` - Mantém a sequência prevista de dependências e reduz bloqueios em macro seguinte.
