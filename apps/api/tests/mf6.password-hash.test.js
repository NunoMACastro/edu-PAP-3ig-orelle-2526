...
describe("BK-MF6-06 / RNF10 - password hash bcrypt", () => {
    ...
    it("só revela conta inativa depois de validar a password correta", async () => {
        const passwordHash = await bcrypt.hash("PalavraPasse12345", BCRYPT_COST);
        const inactiveUser = {
            _id: objectId("user-inactive"),
            email: "inativa@orelle.test",
            role: "cliente",
            passwordHash,
            isActive: false,
        ...
        User.findOne
            .mockReturnValueOnce(queryWithSelect(inactiveUser))
            .mockReturnValueOnce(queryWithSelect(inactiveUser));
        ...
        await expect(
            ...
            statusCode: 403,
            message: "Conta inativa. Contacta a equipa Orélle.",
        });
    });
});
