...
describe("BK-MF5-04 - auditoria biometrica", () => {
    ...
    it("lista logs e alertas apenas para administrador com resposta minimizada", async () => {
        ...
        expect(logs.body.pageInfo).toEqual({
            nextCursor: null,
            hasMore: false,
        });
    ...
    it("continua a auditoria por cursor sem repetir eventos", async () => {
        mockSessionAccount(ROLES.ADMIN);
        BiometricAccessLog.countDocuments.mockResolvedValue(0);
        BiometricAccessLog.create.mockImplementation(async (payload) =>
            makeAuditLog({
                ...payload,
                _id: objectId("665f00000000000000000031"),
                actorId: objectId(payload.actorId),
                subjectUserId: null,
            }),
        );
        const newer = makeAuditLog({
            _id: objectId("665f00000000000000000021"),
            createdAt: new Date("2026-06-22T11:00:00.000Z"),
        });
        const older = makeAuditLog({
            _id: objectId("665f00000000000000000020"),
            createdAt: new Date("2026-06-22T10:00:00.000Z"),
        });
        BiometricAccessLog.find
            .mockReturnValueOnce(querySortLimit([newer, older]))
            .mockReturnValueOnce(querySortLimit([older]));

        const first = await request(createApp())
            .get("/api/admin/biometric-audit/logs?limit=1")
            .set("Cookie", cookieFor(ROLES.ADMIN));
        const second = await request(createApp())
            .get(
                `/api/admin/biometric-audit/logs?limit=1&cursor=${encodeURIComponent(first.body.pageInfo.nextCursor)}`,
            )
            .set("Cookie", cookieFor(ROLES.ADMIN));

        expect(first.status).toBe(200);
        expect(first.body.logs.map(({ id }) => id)).toEqual([
            "665f00000000000000000021",
        ]);
        expect(first.body.pageInfo.hasMore).toBe(true);
        expect(second.status).toBe(200);
        expect(second.body.logs.map(({ id }) => id)).toEqual([
            "665f00000000000000000020",
        ]);
        expect(second.body.pageInfo).toEqual({
            nextCursor: null,
            hasMore: false,
        });
        expect(BiometricAccessLog.find.mock.calls[1][0]).toHaveProperty("$or");
    });

    it("rejeita cursores inválidos antes de registar a leitura", async () => {
        mockSessionAccount(ROLES.ADMIN);

        const response = await request(createApp())
            .get("/api/admin/biometric-audit/logs?cursor=cursor-invalido")
            .set("Cookie", cookieFor(ROLES.ADMIN));
