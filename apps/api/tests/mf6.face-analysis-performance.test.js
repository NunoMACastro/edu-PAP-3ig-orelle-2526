...
describe("MF6/BK-MF6-01 - budget da analise facial", () => {
    ...
    it("mantem resposta publica com performance e sem dados sensiveis", async () => {
        ...
        expect(mocks.analyzeSkinPhotos).toHaveBeenCalledWith({
            frontalPhoto: expect.objectContaining({
                storageKey: frontalPhoto.storageKey,
                mimeType: frontalPhoto.mimeType,
                sizeBytes: frontalPhoto.sizeBytes,
                imageBase64: Buffer.from("frontal-image-bytes").toString(
                    "base64",
                ),
            }),
            perfilPhoto: expect.objectContaining({
                storageKey: perfilPhoto.storageKey,
                mimeType: perfilPhoto.mimeType,
                sizeBytes: perfilPhoto.sizeBytes,
                imageBase64: Buffer.from("perfil-image-bytes").toString(
                    "base64",
                ),
            }),
        });
