...
/**
 * Cria um perfil cosmético mínimo para recomendações MF4.
 *
 * @function makeProfile
 * @param {object} [overrides={}] - Campos a sobrepor no perfil base.
 * @returns {object} Perfil cosmético mock.
 */
...
describe("MF2 - integracao HTTP", () => {
    ...
    it("gera recomendacoes com analise, relatorio e catalogo compativel sem expor dados sensiveis", async () => {
        ...
        Profile.findOne.mockResolvedValueOnce(makeProfile());
    ...
    it("bloqueia geracao de recomendacoes sem perfil cosmetico", async () => {
        FaceAnalysis.findOne.mockReturnValueOnce(queryWithSort(makeAnalysis()));
        FaceReport.findOne.mockReturnValueOnce(queryWithSort(makeReport()));
        ...
        expect(response.body.error.message).toBe("Perfil cosmético obrigatório");
