...
function makeOrder(overrides = {}) {
    return {
        ...
        userId: objectId(userId),
        checkoutKey: `${userId}:${cartId}:${productId}:2:stripe`,
        items: [
            {
                productId: objectId(productId),
                name: "Gel controlo oleosidade",
 ...
 * @param {string} gateway - Gateway usado no checkout.
 * @returns {string} Chave calculada pelo service para o carrinho base.
 */
function makeCheckoutKey(gateway) {
    return `${userId}:${cartId}:${productId}:2:${gateway}`;
}
...
describe("MF3 - comparacao, comercio e stock", () => {
    ...
    it("cria encomenda stub a partir do carrinho e nao confia no total do frontend", async () => {
        ...
        expect(Order.create).toHaveBeenCalledWith(
            expect.objectContaining({
                checkoutKey: makeCheckoutKey("mbway"),
                totalCents: 2598,
                userId,
            }),
        );
    ...
    it("reaproveita encomenda existente para a mesma checkoutKey", async () => {
        const cart = makeCartDoc([makeCartItem()]);
        const order = makeOrder({
            checkoutKey: makeCheckoutKey("paypal"),
            payment: {
                gateway: "paypal",
                status: "pending_manual_confirmation",
                providerReference: "paypal-stub-existing",
                checkoutUrl: null,
                message: "Pagamento pendente de confirmacao manual.",
            },
        });
        Cart.findOne.mockResolvedValueOnce(cart);
        ...
        expect(Cart.deleteOne).toHaveBeenCalledWith({ userId });
    });

    it("envia Idempotency-Key a Stripe usando a checkoutKey do backend", async () => {
        const cart = makeCartDoc([makeCartItem()]);
        const order = makeOrder({
            checkoutKey: makeCheckoutKey("stripe"),
            save: vi.fn().mockResolvedValue(undefined),
        });
        env.stripeSecretKey = "sk_test_orelle";
        vi.stubGlobal(
            "fetch",
        ...
        expect(response.status).toBe(201);
        expect(response.body.order.payment.checkoutUrl).toBe(
        ...
        expect(fetch).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                headers: expect.objectContaining({
                    "Idempotency-Key": makeCheckoutKey("stripe"),
                }),
            }),
        );
        expect(Cart.deleteOne).toHaveBeenCalledWith({ userId });
    ...
    it("marca pagamento Stripe como failed quando o gateway falha apos criar encomenda", async () => {
        const cart = makeCartDoc([makeCartItem()]);
        const order = makeOrder({
            checkoutKey: makeCheckoutKey("stripe"),
            save: vi.fn().mockResolvedValue(undefined),
