...
describe("BK-MF7-01 - consentimento facial explicito", () => {
    ...
    it("recusa body sem consentimento explicito no endpoint", async () => {
        const response = await request(createApp())
            ...
            .send({ accepted: false, version: "face-analysis-v1" });
    ...
    it("bloqueia analise facial quando o consentimento ativo tem finalidade errada", async () => {
        FaceConsent.findOne.mockImplementation(findOnlyWrongPurposeConsent);
    ...
    it("bloqueia visualizacao antes/depois sem finalidade facial cosmetica", async () => {
        FaceConsent.findOne.mockImplementation(findOnlyWrongPurposeConsent);
