...
export async function checkoutController(req, res, next) {
    try {
        const { voucherCode } = validateCheckoutPayload(req.body);
        const order = await checkoutMyCart(req.user.id, { voucherCode });
...
/**
 * Executa o único momento explícito de pagamento simulado.
 *
 * @param {import("express").Request & {user: {id: string}}} req - Pedido autenticado e idempotente.
 * @param {import("express").Response} res - Resposta Express.
 * @param {import("express").NextFunction} next - Middleware de erro.
