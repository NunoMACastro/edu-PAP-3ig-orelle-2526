...
export async function updateCartItemQuantityController(req, res, next) {
    try {
        ...
        const { quantity, variantId } = validateCartQuantityPayload(req.body);
...
export async function removeCartItemController(req, res, next) {
    try {
        const { productId } = validateCartProductParam(req.params);
        const { variantId } = validateCartVariantQuery(req.query);
        const cart = await removeCartItem(req.user.id, productId, variantId);
