 ...
 * @returns {string} Chave estavel para a mesma tentativa de checkout.
 */
function buildCheckoutKey(userId, cart, gateway) {
    ...
    // A chave mistura utilizador, carrinho e gateway para evitar colisao entre clientes.
    return `${userId}:${cartId}:${cartVersion}:${gateway}`;
}

/**
 * Procura uma encomenda ja criada para a mesma tentativa de checkout.
 *
 * @async
 ...
 * @returns {Promise<object|null>} Encomenda reaproveitavel ou null.
 */
...
async function markCheckoutPaymentFailed(order, gateway) {
    order.payment = {
        ...
        checkoutUrl: null,
        message:
            "Nao foi possivel iniciar o pagamento. Tenta novamente antes de alterar o carrinho.",
    };

    // Guardar `failed` deixa rasto operacional sem limpar carrinho nem fingir cobranca.
    await order.save();
}
