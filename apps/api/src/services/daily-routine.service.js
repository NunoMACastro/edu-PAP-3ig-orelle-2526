...
import { assertRecommendationsUnlockedForUser } from "./report-unlock.service.js";
 ...
 * @throws {AppError} Quando nao existem recomendacoes validas suficientes.
 */
export async function generateDailyRoutineForUser(userId) {
    ...
    await assertRecommendationsUnlockedForUser(userId, recommendations);
...
export async function getDailyRoutineForUser(userId) {
    ...
    if (routine?.source === "recommendations") {
        const recommendationIds = (routine.steps ?? [])
            .map((step) => step.recommendationId)
            .filter(Boolean);
        const recommendations = await ProductRecommendation.find({
        ...
        if (recommendationIds.length > 0 && recommendations.length !== recommendationIds.length) {
            throw new AppError(
                423,
