...
export async function reorderFromOrder(userId, orderId) {
    ...
    for (const item of order.items) {
        ...
        let variant = null;
        try {
            variant = resolveProductVariant(product, item.variantId, {
                allowLegacyWithoutVariant: true,
            });
            if (!variant && (product.variants?.length ?? 0) > 0) {
                // Uma encomenda anterior à migração não tinha variante. A
                // recompra escolhe apenas uma variante atualmente disponível,
                // sem alterar o snapshot histórico da encomenda original.
                variant = product.variants.find(
                    (candidate) => candidate.stock >= item.quantity,
                );
                if (!variant) {
                    throw new Error("Sem variante disponível");
                }
            }
        } catch {
            skipped.push({
                ...
                reason: "Variante já não existe",
            });
            continue;
        }

        if (resolveVariantStock(product, variant) < item.quantity) {
            skipped.push({
                productId: item.productId.toString(),
                variantId: item.variantId ?? null,
