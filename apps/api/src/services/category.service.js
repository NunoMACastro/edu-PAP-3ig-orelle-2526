...
/**
 * Lista apenas categorias ativas através de um DTO público minimizado.
 *
 * @async
 * @function listPublicCategories
 * @returns {Promise<Array<{id: string, name: string, slug: string, description: string}>>} Categorias públicas.
 */
export async function listPublicCategories() {
    const categories = await Category.find({ isActive: true }).sort({ slug: 1 });

    return categories.map((category) => {
        const dto = toCategoryResponse(category);
        return {
            id: dto.id,
            name: dto.name,
            slug: dto.slug,
