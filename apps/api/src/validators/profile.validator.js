...
/**
 * Valida listas curtas de restricoes cosmeticas.
 *
 * @function validateRestrictionList
 * @param {string[]} items - Lista normalizada.
 * @param {string} fieldName - Nome do campo para mensagem de erro.
 * @returns {string[]|object} Lista valida ou erro.
 */
function validateRestrictionList(items, fieldName) {
    if (items.length > 12) {
        return { [fieldName]: "Indica no maximo 12 entradas" };
    }

    const tooLong = items.find((item) => item.length > 60);
    if (tooLong) {
        return { [fieldName]: "Cada entrada deve ter no maximo 60 caracteres" };
    }
...
export function validateCreateProfileInput(body) {
    const input = {
        ...
        avoidIngredients: normalizeList(body.avoidIngredients),
        lightMedicalRestrictions: normalizeList(body.lightMedicalRestrictions),
    };
    ...
    for (const field of [
        ...
        const result = validateRestrictionList(input[field], field);
        if (!Array.isArray(result)) Object.assign(errors, result);
    }
...
export function validateUpdateProfileInput(body) {
    ...
    for (const field of [
        ...
        if (field in body) {
            const items = normalizeList(body[field]);
            const result = validateRestrictionList(items, field);
