/**
 * Provider local controlado para simulação de maquilhagem baseline.
 */
import { AppError } from "../middlewares/error.middleware.js";

/**
 * Deriva uma cor estável a partir do ID do produto.
 *
 * @function colorFromProduct
 * @param {object} product - Produto usado na simulação.
 * @returns {string} Cor hexadecimal determinística.
 */
...
/**
 * Escapa texto antes de o inserir no SVG gerado.
 *
 * @function escapeSvgText
 * @param {unknown} value - Valor textual vindo do domínio da aplicação.
 * @returns {string} Texto seguro para interpolação em SVG.
 */
function escapeSvgText(value) {
    return String(value ?? "").replace(/[&<>"']/g, (char) => {
        // O escape evita que nomes de produtos sejam interpretados como markup SVG.
        const entities = {
            "&": "&amp;",
            "<": "&lt;",
            ">": "&gt;",
...
/**
 * Garante que a cor de destaque é um hexadecimal completo.
 *
 * @function normalizeAccentColor
 * @param {string} color - Cor candidata.
 * @returns {string} Cor aceite ou fallback neutro.
 */
...
/**
 * Gera uma imagem SVG embebida em data URL sem expor fotografias privadas.
 *
 * @function createSafeSvgPreviewImage
 * @param {{title: string, subtitle: string, accentColor: string, variant?: string}} input - Conteúdo e estilo do preview.
 * @returns {string} Data URL SVG codificada.
 */
export function createSafeSvgPreviewImage({
    ...
    // O SVG é simbólico: comunica o estado visual sem transportar a fotografia facial real.
