 ...
 * @returns {void}
 * @throws {Error} Quando a condicao esperada e falsa.
 */
...
const [app, layouts, styles, controls, hook] = await Promise.all([
    readWebFile("src/App.jsx"),
    readWebFile("src/components/AppLayouts.jsx"),
...
assert(
    hook.includes("THEME_STORAGE_KEY") && hook.includes("localStorage"),
    "Apenas a preferencia visual deve persistir em localStorage.",
);
assert(
    !hook.includes("sessionStorage"),
    "A preferencia visual nao deve usar sessionStorage.",
);
assert(
...
assert(
    layouts.includes("ThemeControls") &&
        layouts.includes("role-account-menu__themes"),
    "App deve integrar ThemeControls no header.",
);
assert(
    app.includes("PublicLayout") && app.includes("ClientLayout"),
    "App deve montar layouts com header partilhado.",
);
assert(
