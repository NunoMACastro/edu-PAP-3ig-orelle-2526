...
/**
 * Preserva a imagem original sempre que já respeita píxeis e upload. Só usa
 * canvas quando é necessário redimensionar ou reduzir um ficheiro >10 MiB.
 *
 * @async
 * @function compressImageForUpload
 * @param {File} file - Imagem escolhida pelo utilizador.
 * @param {{maxDimension?: number, quality?: number, maxUploadBytes?: number}} [options={}] - Orçamento de compressão.
...
export async function compressImageForUpload(file, options = {}) {
    ...
    try {
        ...
        if (!blob || (blob.size >= file.size && file.size <= maxUploadBytes)) {
            return file;
        }
