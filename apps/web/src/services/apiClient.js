...
export async function apiRequest(path, options = {}) {
    ...
    const response = await fetch(`${API_BASE_URL}${path}`, {
        ...options,
        // O cookie HttpOnly deve acompanhar todos os pedidos da app, mesmo que
        // um caller passe opções próprias ao fetch.
        credentials: "include",
        headers: isFormData
            ? options.headers
            : {
...
export async function apiDownload(path, options = {}) {
    const response = await fetch(`${API_BASE_URL}${path}`, {
        ...options,
        // Relatórios, recibos e exports são recursos autenticados; a sessão
        // deve viajar por cookie HttpOnly e não por token exposto à UI.
        credentials: "include",
    });
